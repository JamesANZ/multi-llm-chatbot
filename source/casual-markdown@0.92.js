...
(function () {
  ...
  md.formatCode = function (match, title, block) {
    ...
    if (title.toLowerCase(title) == "sql") {
      ...
      block = block
        ...
        .replace(/\s\/\/(.*)/gm, " <rem>//$1</rem>");
      block = block.replace(
        /(\s?)(function|procedure|return|exit|if|then|else|end|loop|while|or|and|case|when)(\s)/gim,
        "$1<b>$2</b>$3",
      );
      block = block.replace(
  ...
  md.toc = function (srcDiv, tocDiv, options) {
    ...
    if (options && options.scrollspy) {
      (document.getElementById(options.scrollspy) || document).onscroll =
        function () {
          // get TOC elements, and viewport position
          var list = document.getElementById(tocDiv).querySelectorAll("li");
          var divScroll =
            document.getElementById(options.scrollspy) ||
          ...
          for (var i = 0; i < list.length; i++) {
            ...
            if (pos > 0 && pos < divHeight) {
              list[i].className =
                list[i].className.replace("active", "") + " active"; // classList.add( 'active' );
            } else {
